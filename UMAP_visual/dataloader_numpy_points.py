    ...
    def __init__(self, path, mode='test'):
        ...
        if self.mode == 'test':
            ...
            for i in range(n_folders):
                ...
                npy_files = sorted(glob.glob(os.path.join(folder_path, 'sample_point_feature.npy')))
                for fname in npy_files:
                    npy = np.load(fname)
                    npy = np.mean(npy, axis=0)        #16x12x1024
                    #print(npy.shape)
                    repeat_label = npy.shape[0]
                    #print(repeat_label)
                # 将列表转换为 Numpy 数组
                ...
                # 计算大的patch的小patch样本特征向量的均值
                    ...
                    label = int(parts[-2])
                    #让label重复repeat_label次
                    label = np.tile(label, (repeat_label, 1))
